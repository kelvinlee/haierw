...
postajax = (data)->
	...
	$.ajax
		...
		success: (msg)->
			...
			_gaq.push(['_trackEvent', '移动端', '上传作品', "成功"]);
		error: (msg)->
			$("#uploadsuccess").attr 'src','img/icon-success.png'
			$("#uploadsuccesstext").html "上传失败了<br/>试试再玩一次！"
			_gaq.push(['_trackEvent', '移动端', '上传作品', "失败"]);
...
postajaxPC = (data)->
	...
	$.ajax
		...
		success: (msg)->
			...
			_shareimgurl = msg.src
			BindShare "有些话不愿大声说出来，不妨就写在充满雾气的浴室玻璃上吧！那些话，隐秘而伟大。","http://site.douban.com/haier_waterheater/",_shareimgurl
			_gaq.push(['_trackEvent', '扫描二维码', '上传作品', "成功"]);
		error: (msg)->
			$("#uploadsuccess").attr 'src','img/icon-success.png'
			$("#uploadsuccesstext").html "上传失败了<br/>试试再玩一次！"
			_gaq.push(['_trackEvent', '扫描二维码', '上传作品', "失败"]);
...
showFirstFrame = ->
	...
	if _fromPC
		_gaq.push(['_trackEvent', '扫描二维码', '游戏', '开始']);
		_fromPC = true
		# photoid = getParam "photo_id"
		# localimg = new Image()
		...
		# localimg.onload = ->
		# frompc
		_dom.bglist.b1 = new createjs.Bitmap preload.getResult "frompc"
	...
	bindDecade()
	_gaq.push(['_trackEvent', '移动端', '游戏', '选择年代']);
...
showSecondFrame = ->
	...
	$(".selectres").show()
	createRSQlist()
	_gaq.push(['_trackEvent', '移动端', '游戏', '选择热水器']);
	setTimeout ->
...
createRSQlist = ->
	...
	for i in [1..15]
		...
		item.append "<p>#{rsqarr[(i-1)*2].name}</p>"
		...
		item.click ->
			...
			_gaq.push(['_trackEvent', '移动端', '选择热水器', $("p",this).text()]);
...
GameOver = ->
	...
	$("[data-weixin]").click ->
		$(".weixinshare").show()
		_gaq.push(['_trackEvent', '移动端', '分享', "weixin"]);
	# bind 分享文案.
...
BindShare = (content,url = window.location.href,pic)->
	...
	$("[data-share]").unbind('click').bind 'click', ->
		...
		_gaq.push(['_trackEvent', '移动端', '分享', $(this).data('share')]);
...
ReadyStarGame = ->
	RSQRfadeO = true
	startGame()
	_gaq.push(['_trackEvent', '移动端', '游戏', '开始']);
